...
NGINX_CONF="/etc/nginx/sites-available/basa-app-dev"
...
log "🚀 Starting BASA Development Server Setup..."
...
log "📥 Cloning BASA development repository..."
...
log "🌐 Configuring Nginx for development..."
...
server {
    ...
    add_header Content-Security-Policy "default-src 'self' 'unsafe-eval' 'unsafe-inline' data: blob: http: https:;" always;
}
NGINXEOF
ln -sf "$NGINX_CONF" /etc/nginx/sites-enabled/basa-app-dev
...
EOF
chmod +x /usr/local/bin/deploy-basa-dev /usr/local/bin/stop-basa-dev

success "🎉 Development server setup completed!"
echo ""
echo "📋 Next steps:"
echo "1. Edit $DEV_DIR/.env.development with your actual credentials."
echo "2. Point your DNS for dev.businessassociationsa.com to this server."
echo "3. Run: deploy-basa-dev to deploy the app."
