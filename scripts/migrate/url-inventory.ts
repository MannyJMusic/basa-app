 ...
 *   pnpm migrate:urls --nginx out.conf     # also emit the nginx rules to a file
 *   pnpm migrate:urls --nginx out.conf --same-host   # for the vhost the app itself serves (no self-redirects)
 *
...
/**
 * One exact-match `location` per URL, to be included INSIDE the server block.
 *
 * This was a pair of `map` blocks at first. `map` only lives in the http context,
 * and its hash sizes must be set before the first `map` nginx sees - on the
 * production host CloudPanel's own nginx.conf already declares one, so any later
 * `map_hash_bucket_size` is rejected as a duplicate and the 147-byte keys here do
 * not fit the 64-byte default. Exact locations have no such constraint: they are a
 * sorted array searched by binary search, a few thousand entries is nothing, and
 * the whole thing stays inside the one vhost that uses it.
 */
/**
 * A rule whose destination is the request itself would loop once the app serves
 * this host directly (`$basa_target` empty): "/" -> "/", or the slash-less event
 * variant "/events/x" -> "/events/x". Compared after percent-decoding, because
 * nginx matches the decoded URI while destinations keep the encoded slug.
 */
function isIdentity(key: string, to: string): boolean {
  let decoded = to
  try {
    decoded = decodeURIComponent(to)
  } catch {
    /* malformed escape: treat as different */
  }
  return decoded === key || to === key
}

function nginxConf(entries: Entry[], sameHost: boolean): string {
  const esc = (s: string) => s.replace(/\\/g, '\\\\').replace(/"/g, '\\"')
  const lines: string[] = [
    '# Generated by scripts/migrate/url-inventory.ts - do not edit by hand.',
    '#',
    '# Include this file INSIDE the server block for businessassociationsa.com, after',
    '# setting the host that redirects point at:',
    '#',
    sameHost
      ? '#     set $basa_target "";   # generated with --same-host: the app serves this host itself'
      : '#     set $basa_target https://app.businessassociationsa.com;   # regenerate with --same-host once the app is on the apex',
    '#     include /etc/nginx/basa-redirects.conf;',
    ...(sameHost
      ? ['#', '# Rules whose destination is the request itself were dropped: they would loop.']
      : []),
    '#',
    '# Every URL is listed with and without its trailing slash, so both forms of an old',
    '# link land in the same place. Keys are matched against the decoded, normalised',
    '# URI, so they are stored decoded; destinations keep the encoded slug, which is',
    '# what the importer wrote to Event.slug and what basa-app routes on.',
    '',
  ]
  ...
  for (const e of entries) {
    const body =
      e.disposition.kind === 'gone'
        ? 'return 410;'
        : `return 301 $basa_target${esc((e.disposition as any).to)}$is_args$args;`
    for (const key of variants(e.url)) {
      if (seen.has(key)) continue
      seen.add(key)
      if (sameHost && e.disposition.kind === 'redirect' && isIdentity(key, (e.disposition as any).to)) continue
      lines.push(`location = "${esc(key)}" { ${body} }`)
    }
  }
  lines.push('')
  return lines.join('\n')
...
async function main(): Promise<void> {
  ...
  if (values.nginx) {
    writeFileSync(values.nginx, nginxConf(entries, flags.has('same-host')))
